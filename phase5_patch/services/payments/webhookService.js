...
const claimWebhookEvent = async (event) => {
    ...
    if (event.status === "PROCESSED") {
        return { claimed: false, duplicate: true, processing: false };
    }
    ...
    if (claimed) {
        return { claimed: true, duplicate: false, processing: false };
    }

    return { claimed: false, duplicate: true, processing: true };
};
